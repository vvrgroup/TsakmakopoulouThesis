...
class AttBEVBackbone(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(self.blocks)):
            ...
            x = self.blocks[i](x)
            #print("After block",i, "the shape is", x.shape)

            if self.compress and i < len(self.compression_modules):
                x = self.compression_modules[i](x)

            
            x_fuse = self.fuse_modules[i](x, record_len,4)  #original/comment for model 1 & 3

            #print("After fuse block",i, "the shape is",x_fuse.shape)
            ...
            if len(self.deblocks) > 0:
                ups.append(self.deblocks[i](x_fuse)) #original/ comment for model 1 & 3
                #ups.append(self.deblocks[i](x)) #uncomment for model 1 & 3
            else:
                ups.append(x_fuse)

            #print("After ups block",i, "the shape is", (self.deblocks[i](x_fuse)).shape)
        #print("Len of ups list",len(ups))
